...
const GlobalFeatureListing: React.FC = () => {
  ...
  useEffect(() => {
    const fetchChannels = async () => {
      ...
      setError(null);
      try {
        const response = await fetch('/api/en/top-submit-list/5', {
          headers: {
  ...
  const renderAvatar = (channel: ApiChannel) => {
    ...
    if (channel.sl_title) {
      ...
      const initials = words.length >= 2 ? words[0][0] + words[1][0] : channel.sl_title.slice(0, 2);
      ...
      <div className="min-h-screen bg-gradient-to-b from-blue-100 to-blue-200">
        ...
        <div className="mx-4 mt-6 space-y-4">
          ...
          {!isLoading && !error && channels.length === 0 && (
            <div className="text-center text-gray-500">No channels found.</div>
          )}
          {!isLoading && !error && channels.slice(0, visibleCount).map((channel, idx) => (
            <div key={idx} className="bg-[#b3e6ff] border-2 border-[#33c3ff] rounded-xl p-3 flex items-center space-x-4">
              {/* Avatar */}
              ...
              <div className="flex-1 flex flex-col gap-2">
                <input
                  ...
                  readOnly
                  className="bg-white border border-blue-200 rounded-md px-3 py-2 text-lg font-normal text-gray-700 focus:outline-none cursor-default"
                  style={{ minWidth: 0 }}
                />
                <button
                  onClick={() => handleVisitChannel(channel.sl_link)}
                  className="bg-[#00aaff] hover:bg-[#0099dd] text-white rounded-md py-2 text-lg font-normal transition-colors"
                >
                  Visit Channel
                </button>
              </div>
