...
const MySubmittedLinks: React.FC = () => {
  ...
  const [viewDetails, setViewDetails] = useState<ViewLinkDetails | null>(null);
  ...
  const handleView = async (link: string) => {
    ...
    try {
      const res = await fetch(
        `/api/${l_key.toLowerCase()}/view-list?link_id=${linkObj.id}`,
        {
          credentials: "include",
        }
      );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setShowViewPopup(false);
      setViewDetails(null);
      setSubmittedLinks((prev) => prev.filter((l) => l.id !== viewDetails.id));
    } catch (error) {
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (typeof data === "object" && data !== null) {
        if (
          (data.ok && data.result) ||
          (data.status === 1 && data.success === 1)
        ) {
          setEditSuccess(t("LINK_UPDATED_SUCCESSFULLY"));
          setShowEditToast(true);
          // After edit, reload links and scroll to edited link at top
            ...
            try {
              const response = await axios.get(
                `/api/${l_key.toLowerCase()}/submit-list?telegram_id=${telegramId}`,
                {
                  headers: {
                    Cookie:
                      "ci_session_frontend=a7booq7io5b88ghqll4va8gkvblvgb1i",
                  },
                }
              );
              let newLinks = response.data || [];
              // Move edited link to top
              const idx = newLinks.findIndex(
                (l: SubmittedLink) => l.id === editForm.id
              );
              if (idx > 0) {
                const [edited] = newLinks.splice(idx, 1);
                newLinks = [edited, ...newLinks];
              }
              setSubmittedLinks(newLinks);
              // Scroll to top of table after reload
              setTimeout(() => {
          ...
          reloadAndScroll();
          // auto close edit screen after 3s
  ...
  useEffect(() => {
    if (showFeaturePopup) {
      ...
      const fetchDuration = async () => {
        try {
          const res = await fetch("/api/features-listing-day");
          const data = await res.json();
      ...
      {showFeaturePopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full text-center relative">
            ...
            <h2 className="text-xl font-bold mb-4 text-blue-700">{t("MFL")}</h2>
                ...
                <div className="flex items-center gap-6">
                  <label className="flex items-center gap-2">
                    <input
                      ...
                      onChange={() => setFeatureType("country")}
                    />
                    {t("CFL")}
                  </label>
                  <label className="flex items-center gap-2">
                    <input
                      ...
                      onChange={() => setFeatureType("global")}
                    />
                    {t("GFL")}
                  </label>
                </div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              ...
                              setSelectedCountries((prev) =>
                                prev.filter((id) => id !== c.country_id)
                              );
                            }
                          }}
                        />
                        <span>
                          {c.country_name} (Max: {c.register_allow}, Avai:{" "}
                          {c.allow})
                        </span>
                      </label>
                    ))}
                      ...
                      <td className="py-4 px-4 text-center whitespace-nowrap">
                        <span
                          className={`inline-block rounded-full px-3 py-1 text-sm font-semibold ${
                            getStatusText(link.sl_status) === t("APPROVED")
                              ? "bg-green-100 text-green-800 border border-green-200"
                              : getStatusText(link.sl_status) === t("PENDING")
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-lg w-full text-center relative max-h-[90vh] overflow-y-auto">
            ...
            {showEditForm ? (
              <form onSubmit={handleEditSubmit} className="space-y-4 text-left">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleEditFormChange("sl_type", e.target.value)
                    }
                    className="w-full border rounded px-3 py-2"
                  >
                    <option value="0">{t("GROUP")}</option>
                    <option value="1">{t("CHANNEL")}</option>
                  </select>
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleEditFormChange("sl_title", e.target.value)
                    }
                    className="w-full border rounded px-3 py-2"
                    required
                  />
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      handleEditFormChange("sl_description", e.target.value)
                    }
                    className="w-full border rounded px-3 py-2"
                    required
                  />
                ...
                <div>
                  <label className="block font-medium">{t("LINK")}</label>
                  <input
                    type="text"
                    value={editForm?.sl_link || ""}
                    onChange={(e) =>
                      handleEditFormChange("sl_link", e.target.value)
                    }
                    className="w-full border rounded px-3 py-2"
                    required
                  />
                ...
                {[1, 2, 3].map((i) => (
                  <div key={i}>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleEditFormChange(`sl_tag_${i}`, e.target.value)
                      }
                      className="w-full border rounded px-3 py-2"
                    />
                  </div>
                ...
                {editErrors.form && (
                  <div className="text-red-600 text-sm">{editErrors.form}</div>
                )}
              ...
              <div className="py-8 text-gray-500">{t("LOADING")}</div>
            ) : viewDetails ? (
              ...
              <div className="py-8 text-gray-500">{t("NO_DETAILS")}</div>
            )}
          </div>
        </div>
      )}
